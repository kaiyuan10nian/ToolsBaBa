...
*/
func main() {
	os.Setenv("FYNE_FONT", "./res/msyhl.ttc") //设置env环境
	myApp := app.New()
	...
	//Todo
	//挂载JSON转换功能
	btnJsonTools := widget.NewButton("Json解析/格式化", func() {
		FeatureJSONTools.JsonTool(myApp).Show()
	})
	//挂载RSA加解密功能
	//Todo
	//挂载Unicode功能
	//Todo
	content := container.New(layout.NewHBoxLayout(), btnTimeStamp,btnJsonTools, layout.NewSpacer())
